...
from detect_icon.utils.general import xywh2xyxy, xyxy2xywh
from detect_icon.utils.metrics import fitness
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    cv2.rectangle(img, c1, c2, color, thickness=1, lineType=cv2.LINE_AA) # thickness = tl
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=0.5, thickness=1)[0] 
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, 0.5, [225, 255, 255], thickness=1, lineType=cv2.LINE_AA) # thickness = tf, fontScale = tl /3
